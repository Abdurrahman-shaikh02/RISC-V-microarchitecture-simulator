	...
	switch(control.type){
		...
		case 0b11:
			...
			if(control.branch_opcode == 0b100){
				...
				//jump
				id_ex.PC_next = arithmetic_unit(operand1, operand2 << 1, control.arithmetic_opcode);
				//set the branch taken bit too
				id_ex.cs_ma.branch_taken |= 0b10;
			}else{
				//branch
				log_debug("branch instruction");
				id_ex.PC_next = arithmetic_unit(id_ex.PC, id_ex.imm << 1, control.arithmetic_opcode);
