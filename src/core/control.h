...
typedef struct ex_control{
	uint8_t source1;	//pc or rs1 or rs2 ?		100, 010, 001
	uint8_t source2;	//rs2 or imm?		0, 1
	uint8_t sign;		//unsigned op(0) or signed op(1) ?	0, 1
	uint8_t type;		//arithmetic | logic | less than(slt) | branch compare? 1000, 0100, 0010, 0001
	uint8_t arithmetic_opcode;		//alu opcode 0, 1
	uint8_t logic_opcode;		//logic opcode	01000000, ... 00000001
	uint8_t branch_opcode;		//branch comparison opcode
...
typedef struct ma_control{
	...
	uint8_t jump_or_branch;	//was this a jump (save return address) or a branch (no address needs to be writtenback) ? 0,1
} ma_control;
