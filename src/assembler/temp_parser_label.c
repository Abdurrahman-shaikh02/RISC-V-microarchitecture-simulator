...
// this simple parser will ignore any non alphanumeric character and just parse the alphanumeric ones...
// handles labels too...
// NO UNDERSCORE IN LABELS HOWEVER... (will change this)
char ** get_tokens (char * s){
	...
	int label = 0;
	while(*t){
		if(!(isspace(*t) || isalnum(*t) || *t == ',' || *t == ':')) return NULL;	//invalid character
		if(*t == ';') label = 1;
		t++;
	}
	...
	if(label){
		while(*s == ':'){
			if(isspace(*s) || !isalnum(*s)) return NULL;
			*t = *s;
			t++;
			s++;
		}
		...
		while(*s){
			if(!isspace(*s)) return NULL;	//anything except space after :
		}
	}
